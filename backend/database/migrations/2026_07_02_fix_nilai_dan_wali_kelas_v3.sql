-- ============================================================
-- MIGRATION FIX: Analitik Prestasi + Logbook Review (ASCII-only)
-- Tanggal: 2026-07-02
--
...
--   AnalitikPrestasiController sudah lengkap kodenya dan menunggu
--   tabel ini.
--
-- BUG B - "Guru tidak bisa validasi/kembalikan logbook siswa"
--   Root cause: logbook_praktik.guru_id di-assign otomatis dari
--   rombel_wali_kelas (wali kelas rombel siswa). Rombel yang dibuat
--   di seed sebelumnya (2026-07-01) tidak pernah diisi wali kelasnya,
...
-- ------------------------------------------------------------
-- BAGIAN A.0 - Bersihkan tabel yang mungkin sudah terlanjur
-- terbuat dengan struktur corrupt dari percobaan sebelumnya
-- (encoding issue). Aman di-drop karena belum ada data penting.
-- Urutan: child dulu (nilai_akademik) baru parent (mata_pelajaran)
...
INSERT INTO `mata_pelajaran` (`kode_mapel`, `nama_mapel`, `kelompok`, `jurusan_id`, `kkm`, `status`) VALUES
  ...
  ('PKWU',     'Produk Kreatif dan Kewirausahaan','kejuruan', NULL, 75, 'aktif')
ON DUPLICATE KEY UPDATE `nama_mapel` = VALUES(`nama_mapel`);

INSERT INTO `mata_pelajaran` (`kode_mapel`, `nama_mapel`, `kelompok`, `jurusan_id`, `kkm`, `status`)
SELECT 'PROD-TKJ', 'Produktif TKJ', 'kejuruan', jurusan_id, 78, 'aktif'
FROM `jurusan` WHERE `kode_jurusan` = 'TKJ'
ON DUPLICATE KEY UPDATE `nama_mapel` = VALUES(`nama_mapel`);

INSERT INTO `mata_pelajaran` (`kode_mapel`, `nama_mapel`, `kelompok`, `jurusan_id`, `kkm`, `status`)
SELECT 'PROD-RPL', 'Produktif RPL', 'kejuruan', jurusan_id, 78, 'aktif'
FROM `jurusan` WHERE `kode_jurusan` = 'RPL'
ON DUPLICATE KEY UPDATE `nama_mapel` = VALUES(`nama_mapel`);

INSERT INTO `mata_pelajaran` (`kode_mapel`, `nama_mapel`, `kelompok`, `jurusan_id`, `kkm`, `status`)
SELECT 'PROD-MM', 'Produktif Multimedia', 'kejuruan', jurusan_id, 78, 'aktif'
FROM `jurusan` WHERE `kode_jurusan` = 'MM'
ON DUPLICATE KEY UPDATE `nama_mapel` = VALUES(`nama_mapel`);
...
SET @ta_aktif_id   = (SELECT `tahun_ajaran_id` FROM `tahun_ajaran` WHERE `is_aktif` = 1 LIMIT 1);
SET @ta_semester   = (SELECT `semester_aktif` FROM `tahun_ajaran` WHERE `is_aktif` = 1 LIMIT 1);

INSERT INTO `rombel_wali_kelas` (`rombel_id`, `guru_id`, `tahun_ajaran_id`, `semester`, `status`, `tanggal_mulai`)
SELECT r.`rombel_id`, @guru_demo_id, @ta_aktif_id, @ta_semester, 'aktif', CURDATE()
FROM `rombel` r
