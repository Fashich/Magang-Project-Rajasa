...
import PortalOrtuPublic from './pages/public/PortalOrtuPublic.jsx';

// ── Deteksi route publik (tanpa login) ───────────────────────────────────────
// Portal Orang Tua: /portal-ortu/{token} — dirender TERPISAH dari <App />
// karena tidak perlu heartbeat, service worker khusus auth, atau offline
// redirect yang mengasumsikan sesi login aktif.
const pathMatch = window.location.pathname.match(/^\/portal-ortu\/([a-f0-9-]{20,})\/?$/i);

if (pathMatch) {
  const token = pathMatch[1];
  render(<PortalOrtuPublic token={token} />, document.getElementById('app'));
} else {
  ...
  const getAuthToken = () =>
    localStorage.getItem('presensi_lab_rajasa:auth_token')
